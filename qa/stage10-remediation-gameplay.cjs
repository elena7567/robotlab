...
async function shot(page,label){const p=path.join(dir,label+'.png');await page.screenshot({path:p,timeout:90000});report.screenshots.push(p);}
async function startMotion(page){await page.evaluate(()=>{const g=window.__ROBOTLAB_GAME__;window.__qaMotion=[];window.__qaMotionListener=()=>{const r=g.scene.getScene('Mission10Scene').children.list.find(o=>o.name==='mission10-robot-v2');if(r)window.__qaMotion.push({x:r.x,y:r.y,angle:r.angle,scale:r.scaleX,t:performance.now()});};g.events.on('poststep',window.__qaMotionListener);});}
async function stopMotion(page){return page.evaluate(()=>{window.__ROBOTLAB_GAME__.events.off('poststep',window.__qaMotionListener);return window.__qaMotion;});}
...
async function cancellation(page,seed,touch=false){
 const label=touch?'touch-':'mouse-';
 async function drag(from,to){if(touch){const cdp=await page.context().newCDPSession(page);await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:from.x,y:from.y,id:1}]});await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:to.x,y:to.y,id:1}]});await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await cdp.detach();}else{await page.mouse.move(from.x,from.y);await page.mouse.down();await page.mouse.move(to.x,to.y,{steps:5});await page.mouse.up();}}
 await reset(page,seed);let s=await page.evaluate(inspect);const a=s.lanes.find(l=>l.kind!=='SAFE'),b=s.lanes.find(l=>l.kind==='SAFE');
 await drag(center(a.bounds),center(b.bounds));
 await page.waitForTimeout(40);let after=await page.evaluate(inspect);check(label+'down-other-up-safe-does-not-activate',after.snapshot.pathDecisionIndex===0&&after.snapshot.pathWrongAttempts===0,after);
 await reset(page,seed);s=await page.evaluate(inspect);const wrong=s.lanes.find(l=>l.kind!=='SAFE');
 await drag(center(wrong.bounds),{x:4,y:4});
 await page.waitForTimeout(40);after=await page.evaluate(inspect);check(label+'release-outside-clears-owner',after.pointerOwner===null,after);
 await tap(page,wrong,touch);await page.waitForTimeout(60);after=await page.evaluate(inspect);check(label+'tap-after-cancel-works',after.snapshot.pathWrongAttempts===1&&after.snapshot.pathDecisionIndex===0,after);
}
async function playPath(page,seed,touch,label){
 ...
 for(let round=0;round<3;round++){
  ...
  const initial=s;if(round===0)await shot(page,label+'-round'+round+'-idle'); check(label+'-round'+round+'-robot-not-cropped',s.robot.bounds.x>=0&&s.robot.bounds.right<=page.viewportSize().width&&s.robot.bounds.bottom<=page.viewportSize().height,s.robot.bounds);
  ...
  for(const wrong of wrongs){
   const before=await page.evaluate(inspect);await startMotion(page);await tap(page,wrong,touch,true);await page.waitForTimeout(210);after=await page.evaluate(inspect);const motion=await stopMotion(page);
   ...
   check(label+'-round'+round+'-'+wrong.kind+'-visible-reaction',motion.some(p=>Math.abs(p.x-before.robot.x)>0.5||Math.abs(p.angle-before.robot.angle)>0.25),{before:before.robot,after:after.robot,motion});
   if(round===0&&label.endsWith('PATH_C'))await shot(page,label+'-'+wrong.kind.toLowerCase()+'-reaction');await page.waitForTimeout(600);
  }
  const before=await page.evaluate(inspect),safe=before.lanes.find(l=>l.kind==='SAFE');await startMotion(page);await tap(page,safe,touch);
  ...
  await page.waitForTimeout(550);const safeMotion=await stopMotion(page); check(label+'-round'+round+'-traversal-frame-history',safeMotion.some(p=>Math.abs(p.x-before.robot.x)>2&&Math.sign(p.x-before.robot.x)===Math.sign(center(safe.bounds).x-before.robot.x)),safeMotion); if(round===0)await shot(page,label+'-round'+round+'-safe-motion');await page.waitForTimeout(850);after=await page.evaluate(inspect);
...
(async()=>{
 ...
 try{
  const matrix=[[1280,720,true],[844,390,true],[740,360,false],[1024,768,false],[1600,900,false],[1920,1080,false],[915,412,false],[568,320,false]];
  for(const [width,height,allConfigs] of matrix){
   ...
   if(width===1280||width===844)await cancellation(page,Object.values(seeds)[0],touch);
