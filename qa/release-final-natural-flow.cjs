...
(async()=>{
 ...
 const context=await browser.newContext({viewport:{width:1280,height:720},hasTouch:false,reducedMotion:'reduce'});
 ...
 try{
  ...
  check('production-bundle-loaded', scripts.some(x=>x.includes('/assets/index-') && x.endsWith('.js')), scripts);
 ...
 finally{fs.writeFileSync('docs/qa/release-final-natural-flow.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));await browser.close();}
